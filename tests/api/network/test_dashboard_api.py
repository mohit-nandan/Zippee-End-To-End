...
class TestDashboardNetworkAPI:
    ...
    def test_brands_api(self, network_page, interceptor, web_cfg):
        ...
        if not endpoints:
            pytest.skip("No endpoints defined for 'brands' — add paths to api_test_data.py")
        _watch_and_navigate(
            interceptor, network_page, endpoints,
            lambda: BrandsPage(network_page).go_to("brands", web_cfg["dashboard_url"]),
        )
